  ...
  config = lib.mkIf cfg.enable {
    ...
    systemd.services."hdd-replication" = {
      script = ''
        ...
        # zroot -> HDD2
        ${pkgs.sanoid}/bin/syncoid -r --skip-parent --no-sync-snap --exclude=zroot/bitcoin --exclude=zroot/root --exclude=zroot/nix zroot HDD2/zroot
        # --exclude will be deprecated in favor of --exclude-datasets

        # nvme/Samba -> HDD1
        ${pkgs.sanoid}/bin/syncoid -r --skip-parent --no-sync-snap nvme/Samba HDD1/Samba

        # nvme/Backup -> HDD1
        ${pkgs.sanoid}/bin/syncoid -r --skip-parent --no-sync-snap nvme/Backup HDD1/Backup

        # nvme/cloudton -> HDD1
        ${pkgs.sanoid}/bin/syncoid -r --skip-parent --no-sync-snap nvme/cloudton HDD1/cloudton
